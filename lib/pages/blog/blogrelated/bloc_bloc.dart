...
class BlogCubit extends Cubit<BlogState> {
  ...
  Future<void> loadBlogs() async {
    try {
      final response = await http.get(Uri.parse('http://localhost:9090/api/blogs'));
  ...
  Future<void> deleteBlog(String blogId) async {
    try {
      final response = await http.delete(Uri.parse('http://localhost:9090/api/blogs/$blogId'));
  ...
  Future<void> addBlog(String title, String description) async {
    try {
      final response = await http.post(
        Uri.parse('http://localhost:9090/api/blogs'),
